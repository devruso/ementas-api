...
const createServiceWithoutDb = () => {
    const service = Object.create(ComponentService.prototype) as ComponentService;
    (service as any).docxSignatureEmbedder = new DocxSignatureEmbedder();
    return service;
};

const createSignatureAsset = async () => {
    const { Jimp, rgbaToInt } = await import('jimp');
    const width = 420;
    const height = 120;
    const image = new Jimp({ width, height, color: rgbaToInt(255, 255, 255, 0) });
    const ink = rgbaToInt(20, 32, 52, 255);

    const drawPoint = (x: number, y: number) => {
        for (let offsetY = -1; offsetY <= 1; offsetY += 1) {
            if (y + offsetY >= 0 && y + offsetY < height) {
                image.setPixelColor(ink, x, y + offsetY);
            }
        }
    };
    ...
    for (let x = 24; x < width - 24; x += 1) {
        ...
        drawPoint(x, Math.round(64 + (18 * Math.sin(progress * Math.PI * 8))));
    }

    for (let x = 90; x < width - 45; x += 1) {
        const progress = x / width;
        drawPoint(x, Math.round(76 + (9 * Math.sin(progress * Math.PI * 13))));
    }
...
const main = async () => {
    ...
    const signatureAsset = await createSignatureAsset();

    samples.forEach(({ fileName, data }) => {
        const buffer = (service as any).fillDocxTemplateFromBase(data, signatureAsset) as Buffer;
...
main().catch((error) => {
    console.error('[docx-samples] generation failed:', error);
    process.exit(1);
});
